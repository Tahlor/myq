...
The direct REST facade exposes explicit open/close operations only; it does not use a toggle. Read-only status is available globally at `GET /status` and for a single account at `GET /accounts/{account_id}/status`; a read-only action preflight is available at `GET /accounts/{account_id}/doors/{door_opener_id}/preflight/{open|close}`. The action endpoints first read the named door and refuse an unknown, transitional, or offline state; they send at most one action and return success only after a fresh status read verifies the requested state. Same-state requests are verified no-ops, and an unverified post-action state is reported as an error.
